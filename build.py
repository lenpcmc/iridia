...
def buildNumber(filename :str, numAtoms :int = 1000, fmax: float = 0.1) -> Atoms:
    chgnet = CHGNet.load()
    structure = Structure.from_file(filename)
    relaxedStruct = relaxStruct(structure, fmax = fmax)["trajectory"].atoms
...
def buildArray(filename :str, repeat :np.typing.ArrayLike = [1,1,1], fmax: float = 0.1) -> Atoms:
    chgnet = CHGNet.load()
    structure = Structure.from_file(filename)
    relaxed = relaxStruct(structure, fmax = fmax)
...
def relaxStruct(structure :Structure, fmax: float = 0.1) -> dict:
