...
def dC(rvec: np.ndarray[float], ai: str, aj: str = None, cutoff: float = 10., n: int = 0) -> np.ndarray[float]:
    ...
    np.putmask(fGaussCharges, np.isclose(np.broadcast_to(rnorm, fGaussCharges.shape), 0.), 0.)
    #np.putmask(fGaussCharges, np.broadcast_to(rnorm > cutoff, fGaussCharges.shape), 0.)
    #fGaussCharges * Tap(rvec, cutoff)

    idx = np.arange(fGaussCharges.shape[0])
    #fGaussCharges[idx, idx, :] = 0.
...
def PQEqForce(position: np.ndarray[float], sposition: np.ndarray[float] = None, elem: str = None, charge: np.ndarray[float] = None, cell: Cell = None, pbc: list[bool] = None, n: int = 0) -> np.ndarray[float]:
    ...
    if (charge is None):
        charge: np.ndarray = np.zeros(len(position))
    ...
    # Core-Shell Charges
    Z: np.ndarray = np.array([ [params["Z", e]] for e in elem ])
    q: np.ndarray = np.array(charge) .reshape(Z.shape) + Z

    # Core-Shell Distances
    ricjc: np.ndarray = get_distances(pos, pos, cell, pbc)
    ricjs: np.ndarray = get_distances(pos, spos, cell, pbc)
    
    # Core-Shell Forces
    Ficjc: np.ndarray = -1. * dC(ricjc, elem, n = n) * np.stack([ q*q.T ], axis = -1)
    Ficjs: np.ndarray = -1. * dC(ricjs, elem, n = n) * np.stack([ q*Z.T ], axis = -1)
    Fext = Ficjc - Ficjs
